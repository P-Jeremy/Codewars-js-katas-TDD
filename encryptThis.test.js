...
describe("Encrypt this", function() {
  ...
  describe('Two letters word', () => {
    it('Should return "65l" when given "Al" ', () => {
      const result = encryptThis('Al')
      expect(result).toBe('65l')
    })
  })

  describe('Three letters word', () => {
    it('Should return "98ra" when given "bar" ', () => {
      const result = encryptThis('bar')
      expect(result).toBe('98ra')
    })
  })

  it('Should revert the second and last letter of the word', () => {
    expect(encryptThis('othello')).toBe('111ohellt')
    expect(encryptThis('Avion')).toBe('65niov')
    expect(encryptThis('Koala')).toBe('75aalo')
    expect(encryptThis('Alm')).toBe('65ml')
  })
